...
class WTLClient {
    ...
    try {
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          console.log(
            `❌ Failed ${endpoint}: ${error.response?.status || error.message}`
          );
          continue;
        }
      }
  ...
  async getTasks(projectId?: string): Promise<WTLResponse<any[]>> {
    try {
      ...
      const endpoints = projectId
        ...
        : [
            `/task/list?range=[0,1000]&sort=["id", "ASC"]`,
            "/assignment/list?range=[0,1000]",
          ];
  ...
  async getUsers(): Promise<WTLResponse<any[]>> {
    try {
      // Właściwy endpoint zgodnie z dokumentacją WTL API
      const endpoint = '/user/list?range=[0,1000]&sort=["id", "ASC"]';
  ...
  async getLessons(trainingId?: string): Promise<WTLResponse<any[]>> {
    try {
      ...
      const endpoints = trainingId
        ...
        : [
            `/lesson/list?range=[0,1000]&sort=["order", "ASC"]`,
            "/lessons",
            "/lesson",
          ];
  ...
  async verifyUserByEmail(email: string): Promise<WTLResponse<any>> {
    try {
      ...
      console.log(
        `❌ User verification failed for ${email}: ${
   ...
   */
  async verifyUserWithRole(
    email: string
  ): Promise<WTLResponse<WTLUserWithRole>> {
    try {
  ...
  private async determineUserRole(
    ...
    try {
      ...
      try {
        const teacherCourses = await this.client.get(
          `/training/list?filter=[{"field": "instructor_id", "type": "equals", "value": "${userId}"}]`
        );
        if (
          teacherCourses.data &&

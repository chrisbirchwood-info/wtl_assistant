...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!otp || typeof otp !== "string") {
      return NextResponse.json({ error: "OTP jest wymagany" }, { status: 400 });
    }
    ...
    // ALE: superadmin zachowuje swoją rolę!
    if (supabaseUser.role !== "teacher" && supabaseUser.role !== "superadmin") {
      console.log(
        `🔒 User role is not 'teacher' or 'superadmin', ensuring role is 'student'`
      );
      supabaseUser.role = "student";
    }

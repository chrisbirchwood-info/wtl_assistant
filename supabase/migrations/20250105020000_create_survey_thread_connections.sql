-- Migration: Survey-Thread Connection System
-- Date: 2025-01-05 02:00:00
-- Purpose: Enable linking survey responses to student threads

-- Table to connect survey forms with threads (waiting for responses)
...
-- Policy: Teachers can create connections for survey responses from their forms
CREATE POLICY "Teachers can create survey thread connections" ON public.thread_survey_connections
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 
      FROM public.survey_responses sr
      JOIN public.survey_forms sf ON sf.form_id = sr.form_id
      WHERE sr.id = thread_survey_connections.survey_response_id
